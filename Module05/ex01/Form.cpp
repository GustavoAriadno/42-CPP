...
Form::Form(const std::string _name, int reqToSign, int reqToExec) :
	...
	if (reqToSign < HIGHEST_GRADE || reqToExec < HIGHEST_GRADE) throw Form::GradeTooHighException();
	if (reqToSign > LOWEST_GRADE || reqToExec > LOWEST_GRADE) throw Form::GradeTooLowException();
}
...
Form::Form(const Form &b) :
	...
	if (b.requiredToSign < HIGHEST_GRADE || b.requiredToExec < HIGHEST_GRADE) throw Form::GradeTooHighException();
	if (b.requiredToSign > LOWEST_GRADE || b.requiredToExec > LOWEST_GRADE) throw Form::GradeTooLowException();
}
...
void				Form::beSigned(Bureaucrat b) {
	if (this->getRequiredToSign() < b.getGrade()) throw Form::GradeTooLowException();
	this->isSigned = true;
}
